...
config_files = [
    ...
    ('src/parquet.pkg', 'v {major}.{minor}.{patch}'),
    ('src/parquet.pkg', 'd Distribution-Date: {date:%Y%m%d}'),
    ('src/stata.toc', 'v {major}.{minor}.{patch}'),
    ('docs/parquet.sthlp', 'version {major}.{minor}.{patch} {date:%d%b%Y}'),
    ('src/plugin/parquet.h', 'define SPARQUET_VERSION "{major}.{minor}.{patch}"'),
    ('src/plugin/parquet.cpp', '@date {date:%d %b %Y}'),
    ('src/plugin/parquet.cpp', 'Version: {major}.{minor}.{patch}')
]
